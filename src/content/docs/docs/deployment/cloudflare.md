...
The deployment workflow provisions infrastructure for ZPan itself; it does not choose where user files belong. Add R2, S3, B2, Tigris, or another S3-compatible bucket under **Admin Console → Storages**, then configure browser CORS as described in [Object storage](/docs/site-management/storage/).
